...
if($token_time==0){
	...
	$sql ="SELECT `hesprrs_option_sub`.`sno_option_sub` as 'sub_pk' ,`hesprrs_option_main`.`sno_option_main` as 'main_id',
	`hesprrs_option_sub`.`option_sub_order` as 'sub_id',`hesprrs_option_sub`.`name` as 'sub_name', `hesprrs_members`.`name` as 'supervisor',`hesprrs_members`.`sno_members` as 'supervisor_id', 
	...
	WHERE `hesprrs_option_main`.`activate_yyy` =".$year." and  `hesprrs_option_sub`.`sno_option_sub` ='".$sub_pk."'";
