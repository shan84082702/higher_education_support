...
if($token_time==0){
	...
	if($row['supervisor_strategies']==$id){
		...
		VALUES ('審核未通過' , '".$message."','".$id."', '".$roww['supervisor_project']."','".$sno_option_projects."')";
		$resultt = $conn->query($sqll) ;

		$sqll ="UPDATE `hesprrs_projects_data` SET `review_state` = '3' WHERE `sno_option_projects` = '".$sno_option_projects."'";
		$resultt = $conn->query($sqll) ;
	}
	else{
		//新增站內信
		$sqll ="INSERT INTO `hesprrs_messages`(`type`, `message`, `sno_source`, `sno_target`, `sno_projects`)
		VALUES ('審核未通過' , '".$message."','".$id."', '".$row['supervisor_strategies']."','".$sno_option_projects."')";
		$resultt = $conn->query($sqll) ;
