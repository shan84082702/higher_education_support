...
if($token_time==0){
	...
	$sql ="SELECT DISTINCT
		`hesprrs_option_projects`.`sno_option_projects` as 'project_pk',
		`hesprrs_option_projects`.`name` as 'name',
		`hesprrs_option_projects`.`activate_date` as 'act_date',
		`hesprrs_option_projects`.`activate_yyy` as 'year',
		...
		concat(`hesprrs_edu_indicators`.`name`,'/',`hesprrs_edu_indicators_sub`.`name`,'/',`hesprrs_edu_indicators_detail`.`name`) as 'edu',
		...
		AND
		(`hesprrs_projects_editor`.`sno_members` =  '".$id."'
		OR `hesprrs_option_main_editer`.`sno_member` = '".$id."'
		OR `hesprrs_option_sub_editor`.`sno_member` = '".$id."'
		OR `hesprrs_option_strategies_editor`.`sno_members` = '".$id."'
		OR ('1' IN (SELECT `sno_roles` FROM `hesprrs_members` WHERE `sno_members` = '".$id."' )) )";
