...
@$role_diecription = $_POST["role_diecription"];
@$project_edit = $_POST["project_edit"];
@$strategiest_summary = $_POST["strategiest_summary"];
@$member_manage = $_POST["member_manage"];
@$data_manage = $_POST["data_manage"];
@$view_data = $_POST["view_data"];
@$review = $_POST["review"];
include('fig.php');
if($token_time==0){
	$sql ="UPDATE `hesprrs_roles` SET 
	`name`='".$role_name."',`description`='".$role_diecription."',`project_edit`='".$project_edit."',
	`strategiest_summary`='".$strategiest_summary."',`member_manage`='".$member_manage."',`data_manage`='".$data_manage."',`view_data`='".$view_data."', `review` = '".$review."'
	WHERE `sno_roles` = '".$sno_role."'";
