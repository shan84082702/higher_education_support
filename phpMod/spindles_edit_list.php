...
if($token_time==0){
	...
	WHERE `activate_yyy` =".$year." and  hesprrs_option_main.`sno_option_main` =".$main_pk;
	...
	while($row = $result->fetch_array(MYSQLI_ASSOC)){
		...
		WHERE `sno_option_main`=".$out1['son_id']." and `sno_option_main`=".$main_pk." and  `sno_member` !=".$out1['supervisor_id'] ." and `hesprrs_members`.`sno_roles` !=1";
...
////
		$sqll ="SELECT `hesprrs_members`.`sno_members`, `hesprrs_members`.`name`, `hesprrs_members`.`sno_roles` FROM `hesprrs_review` 
		LEFT JOIN `hesprrs_members` ON `hesprrs_members`.`sno_members` = `hesprrs_review`.`sno_members` 
		WHERE `sno_option_main` = '".$out1['son_id']."' and `hesprrs_members`.`sno_roles`!='1'";
		$resultt = $conn->query($sqll) ;
		$third_out=array();
		while($row = $resultt->fetch_array(MYSQLI_ASSOC)){
			$third_out1['sno_members']=$row['sno_members'];
			$third_out1['name']=$row['name'];
			$third_out1['sno_roles']=$row['sno_roles'];
